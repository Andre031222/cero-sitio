...
public class DescargaController {
    ...
    public Object zip(@Query("modulos") String lista) {
        if (lista == null || lista.isBlank()) {
            throw new HttpException(400, "hace falta ?modulos=corvo-core,corvo-data");
        }
    ...
    /**
     * Un nombre que diga qué hay dentro. Seis zip llamados {@code corvo.zip} en la carpeta de
    ...
    private static String nombreZip(Set<String> resueltos) {
        if (resueltos.size() == ModulosController.cuantos()) {
            return "corvo-" + VERSION + "-completo.zip";
        }
        return "corvo-" + VERSION + "-" + resueltos.size() + "-modulos.zip";
    }

    private static String leeme(Set<String> modulos) {
        StringBuilder sb = new StringBuilder();
        sb.append("Corvo ").append(VERSION).append("\n");
        sb.append("https://corvo.ginit.dev\n\n");
        sb.append("Este zip trae:\n");
        for (String m : modulos) {
            sb.append("  - ").append(nombreJar(m)).append('\n');
        }
        sb.append("\nLas dependencias entre módulos ya están resueltas: si pediste corvo-view,\n");
        sb.append("dentro van también corvo-core y corvo-http, que es lo que necesita.\n");
        ...
        for (String m : modulos) {
            sb.append("  mvn install:install-file -Dfile=").append(nombreJar(m))
              .append(" -DgroupId=dev.ginit.corvo -DartifactId=").append(m)
        ...
        sb.append("  git clone https://github.com/ginit-dev/corvo && cd corvo && ./corvo install\n");
        return sb.toString();
    }
}
